...
def home(request):
    df_lc = pd.DataFrame(list(LC.objects.all().values('lc_id', 'lc_name','lc_color').order_by('lc_id'))) # Get the LC dataset data sorted by lc_id
...
def deforestation(request):
    ...
    return render(request, 'scapadmin/deforestation.html', context={'chart': chart,'lcs': json.dumps(lcs),'lc_data':lcs})
