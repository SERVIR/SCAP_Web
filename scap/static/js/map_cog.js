...
function  redraw_based_on_year() {
    ...
    var fc_scale_range_right = "";
        ...
        var base_thredds = 'https://titiler.servirglobal.net/cog/tiles/{z}/{x}/{y}?tileMatrixSetId=WebMercatorQuad&scale=1&url=/mnt/scap/cogs/' + thredds_dir + '/1/';
        if (map_modal_action == 'deforestation_targets') { // Forest Cover usecase
            ...
            base_thredds = 'https://titiler.servirglobal.net/cog/tiles/{z}/{x}/{y}?tileMatrixSetId=WebMercatorQuad&scale=1&url=/mnt/scap/cogs/' + thredds_dir + '/1/';
            //Generate the WMS URLs from available data
            if (selected_dataset_left === selected_dataset_right) {
                pri_over_style = 'accent_r';
                sec_over_style = 'rplumbo';
                sec_under_style = 'rdylbu';
            } else {
                pri_over_style = 'accent_r';
                sec_over_style = 'rplumbo';
                sec_under_style = 'gnuplot_r';
            }


            primary_overlay_url = `${base_thredds}/${selected_dataset_left}/${thredds_dir}.1.${selected_dataset_left}.${selected_year}.tif&unscale=false&resampling=nearest&transparent=true&colormap_name=` + pri_over_style + `&return_mask=true&nodata=0&crs=EPSG:3857`;
            primary_underlay_url = `${base_thredds}/${selected_dataset_right}/${thredds_dir}.1.${selected_dataset_right}.${comparison_year}.tif&unscale=false&resampling=nearest&transparent=true&colormap_name=` + pri_over_style + `&return_mask=true&nodata=0&crs=EPSG:3857`;
            secondary_overlay_url = `${base_thredds}/${selected_dataset_right}/${thredds_dir}.1.${selected_dataset_right}.${comparison_year}.tif&unscale=false&resampling=nearest&transparent=true&colormap_name=` + sec_over_style + `&return_mask=true&nodata=0&crs=EPSG:3857`;
            secondary_underlay_url = `${base_thredds}/${selected_dataset_left}/${thredds_dir}.1.${selected_dataset_left}.${selected_year}.tif&unscale=false&resampling=nearest&transparent=true&colormap_name=` + sec_under_style + `&return_mask=true&nodata=0&crs=EPSG:3857`;
            ...
            scale_range = "1,550";
            //Generate the WMS URLs from available data
            base_thredds = 'https://titiler.servirglobal.net/cog/tiles/{z}/{x}/{y}?tileMatrixSetId=WebMercatorQuad&scale=1&url=/mnt/scap/cogs/' + thredds_dir;
            primary_overlay_url = `${base_thredds}/${thredds_dir}.1.${selected_dataset_left_agb}_cog.tif&unscale=false&resampling=nearest&transparent=true&colormap_name=pink_r&return_mask=true&nodata=0&crs=EPSG:3857`;
            secondary_overlay_url = `${base_thredds}/${thredds_dir}.1.${selected_dataset_right_agb}_cog.tif&unscale=false&resampling=nearest&transparent=true&colormap_name=pink_r&return_mask=true&nodata=0&crs=EPSG:3857`;
            // Defining styles/palettes based on dataset selections
     ...
     primary_overlay_layer = L.tileLayer(primary_overlay_url,{ pane: 'left'});


     primary_underlay_layer = L.tileLayer(primary_underlay_url,{ pane: 'left'})
     secondary_overlay_layer = L.tileLayer(secondary_overlay_url,{ pane: 'right'})

     secondary_underlay_layer = L.tileLayer(secondary_underlay_url,{ pane: 'right'})
...
function redraw_map_layers() {
    ...
    // if (window.location.href.indexOf("/map-cog/") > -1)
    //     get_stats_for_map();
}
//Get dataset names
...
function init_map() {
    ...
    if(country_layer!=undefined && window.location.href.indexOf("/pilot/") > -1){
     ...
     if(country_layer!=undefined && window.location.href.indexOf("/map-cog/") > -1){
        country_layer.addTo(map);
    }
    if (aoi_layer!=undefined && window.location.href.indexOf("/aoi/") > -1) {
        aoi_layer.addTo(map);
    }
    if (aoi_layer!=undefined && window.location.href.indexOf("/map-cog/") > -1) {
        aoi_layer.addTo(map);
    }
...
function zoomtoArea(id){
    if (id!==0) {
        window.location = window.location.origin + "/map-cog/" + id + "/";
